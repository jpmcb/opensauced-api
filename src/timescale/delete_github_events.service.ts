...
export class DeleteGithubEventsService {
  ...
  async genDeleteHistogram(options: DeleteHistogramDto): Promise<DbDeleteGitHubEventsHistogram[]> {
    ...
    queryBuilder
      ...
      .where(`:start_date::TIMESTAMP >= "delete_github_events"."event_time"`, { start_date: startDate })
      .andWhere(`:start_date::TIMESTAMP - :range_interval::INTERVAL <= "delete_github_events"."event_time"`, {
    ...
    if (options.repo) {
      queryBuilder.andWhere(`LOWER("delete_github_events"."repo_name") IN (:...repoNames)`, {
    ...
    if (options.repoIds) {
      queryBuilder.andWhere(`"delete_github_events"."repo_id" IN (:...repoIds)`, {
